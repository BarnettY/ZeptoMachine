...
int CTMCM3216Motor::GetTargetPos(int& pos)
{
    return m_pCtrl ? m_pCtrl->ReadTargetPos(m_AxisID, pos) : -1;
}
...
BOOL CTMCM3216Motor::IsMoving()
{
    ...
    return m_pCtrl->GetState(m_AxisID) == MS_MOVING ||
           m_pCtrl->GetState(m_AxisID) == MS_RUN_VEL ||
           m_pCtrl->GetState(m_AxisID) == MS_HOMING;
}
