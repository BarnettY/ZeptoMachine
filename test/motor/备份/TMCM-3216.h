...
{
    uint32_t id;         // CAN ID
    uint8_t  data[8];    // 8字节数据域
    uint8_t  len;        // 固定为8
};

// TMCL 状态码枚举
enum class TMCL_Status : uint8_t
{
    SUCCESS                     = 0x00,  // 执行成功
...
{
    bool          valid;          // 帧是否合法
    uint8_t       moduleAddr;     // 模块地址
    uint8_t       status;         // 状态码(0=正常)
    int32_t       value;          // 解析出的32位数据
    uint8_t       command;        // 原始指令码
    uint8_t       type;           // 类型参数
    uint8_t       axis;           // 轴号
    
    bool isSuccess() const { return status == static_cast<uint8_t>(TMCL_Status::SUCCESS); }
    bool hasError() const { return status != static_cast<uint8_t>(TMCL_Status::SUCCESS); }
};

// 电机状态标志位
enum class MotorStatusFlag : uint32_t
{
...
struct IO_Status
{
    uint8_t digitalInputs;   // 数字输入状态 (bit0-3: DI0-DI3)
    uint8_t digitalOutputs;  // 数字输出状态 (bit0-3: DO0-DO3)
    uint16_t analogInput0;   // 模拟输入0值 (0-4095)
    uint16_t analogInput1;   // 模拟输入1值 (0-4095)
    uint16_t analogOutput0;  // 模拟输出0值 (0-4095)
    uint16_t analogOutput1;  // 模拟输出1值 (0-4095)
};

// 运动状态结构体
struct MotionStatus
{
    int32_t actualPosition;   // 当前位置
    int32_t actualVelocity;   // 当前速度
...
public:
    // ---------------------- 底层通用打包接口 ----------------------
    static TMCL_CAN_Frame packFrame(uint8_t moduleAddr, uint8_t cmd, uint8_t type, uint8_t axis, int32_t value)
    {
    ...
    static TMCL_RxResult parseFrame(const uint8_t* rxData, uint8_t dataLen)
    {
        ...
        res.value = static_cast<int32_t>(
            (static_cast<uint32_t>(rxData[4]) << 24) |
            (static_cast<uint32_t>(rxData[5]) << 16) |
            (static_cast<uint32_t>(rxData[6]) << 8)  |
            static_cast<uint32_t>(rxData[7])
        );
    ...
    {
        return (statusValue & static_cast<uint32_t>(flag)) != 0;
    }
    ...
    {
        return static_cast<uint16_t>(result.value & 0xFFFF);
    }
    ...
    // ---------------------- 固件版本解析 ----------------------
    static void parseFirmwareVersion(const TMCL_RxResult& result, uint8_t& major, uint8_t& minor, uint16_t& build)
    {
        uint32_t version = static_cast<uint32_t>(result.value);
        major = static_cast<uint8_t>((version >> 24) & 0xFF);
        minor = static_cast<uint8_t>((version >> 16) & 0xFF);
        build = static_cast<uint16_t>(version & 0xFFFF);
    }
    ...
    {
        return packFrame(addr, TMCL_SAP, AXIS_MAX_CURRENT, axis, static_cast<int32_t>(current));
    }

    static TMCL_CAN_Frame setMicrostep(uint8_t addr, uint8_t axis, uint16_t microStep)
    {
        return packFrame(addr, TMCL_SAP, AXIS_MICROSTEP_RES, axis, static_cast<int32_t>(microStep));
    }

    static TMCL_CAN_Frame setHoldingCurrent(uint8_t addr, uint8_t axis, uint8_t current)
    {
        return packFrame(addr, TMCL_SAP, AXIS_HOLDING_CURRENT, axis, static_cast<int32_t>(current));
    }

    static TMCL_CAN_Frame setStandbyCurrent(uint8_t addr, uint8_t axis, uint8_t current)
    {
        return packFrame(addr, TMCL_SAP, AXIS_STANDBY_CURRENT, axis, static_cast<int32_t>(current));
    }
    ...
    {
        return packFrame(addr, TMCL_SAP, AXIS_STALL_THRESHOLD, axis, static_cast<int32_t>(threshold));
    }
    ...
    {
        return packFrame(addr, TMCL_SAP, AXIS_RFS_MODE, axis, static_cast<int32_t>(mode));
    }
    ...
    static TMCL_CAN_Frame packFrameEx(uint8_t moduleAddr, uint8_t cmd, uint8_t type, uint8_t axis, int32_t value)
    {
    ...
    {
        return packFrame(addr, TMCL_SIO, IO_AOUT0, 0, static_cast<int32_t>(value));
    }

    static TMCL_CAN_Frame setAOUT1(uint8_t addr, uint16_t value)
    {
        return packFrame(addr, TMCL_SIO, IO_AOUT1, 0, static_cast<int32_t>(value));
    }
    ...
    {
        return packFrame(addr, TMCL_SGP, GLOBAL_MODULE_ADDRESS, 0, static_cast<int32_t>(newAddr));
    }
